...
function webhookToRecoveryCase(payload: RazorpayWebhookPayload): RecoveryCase {
  ...
  if (payload.event === 'payment.failed') {
    ...
    return {
      ...
      amountAtRisk: Math.round(p.amount / 100),
      currency: 'INR',
      ...
      interventionReasoning: `Webhook: payment.failed — ${p.error_description}`,
      ...
      auditTrail: [
        {
          ...
          details: `Razorpay webhook: payment.failed — ${p.error_description}`,
  ...
  if (payload.event === 'subscription.halted') {
    ...
    const pmtAmount = payload.payload.payment?.entity.amount ?? 149900;
    const channel: ActionChannel = 'SMART_MANDATE_RETRY';
    return {
      id,
      paymentId: payload.payload.payment?.entity.id ?? `pay_${id}`,
      customerName: s.notes?.customer_name ?? 'Unknown Customer',
      customerPhone: s.notes?.customer_phone ?? '+91-0000000000',
      customerEmail: '',
      preferredLanguage: 'Hinglish',
      amountAtRisk: Math.round(pmtAmount / 100),
      currency: 'INR',
      leakVector: 'SUBSCRIPTION_FAIL',
      status: 'DETECTED',
      failureCategory: 'SOFT_DECLINE',
      failureCode: payload.payload.payment?.entity.error_code ?? 'EMANDATE_FAIL',
      failureReason: payload.payload.payment?.entity.error_description ?? 'eMandate debit failed',
      issuingBank: 'HDFC',
      attemptsCount: 0,
      maxAttemptsAllowed: 3,
      recommendedChannel: channel,
      interventionReasoning: `Webhook: subscription.halted — ${s.payment_method} mandate failed`,
      totalAmountRecovered: 0,
      discountOfferedPct: 0,
      bankDowntimeDetected: false,
      createdAt: now,
      ...
      auditTrail: [
        {
          ...
          details: `Razorpay webhook: subscription.halted — ${s.payment_method} mandate failed`,
          complianceCheck: { dndCompliant: true, maxAttemptsRespected: true, disputeCheckPassed: true },
        },
      ],
    };
  ...
  if (payload.event === 'order.abandoned') {
    ...
    return {
      ...
      customerPhone: o.notes?.customer_phone ?? '+91-0000000000',
      customerEmail: '',
      preferredLanguage: 'Hinglish',
      amountAtRisk: Math.round(o.amount / 100),
      currency: 'INR',
      leakVector: 'CHECKOUT_ABANDON',
      status: 'DETECTED',
      failureCategory: 'INTENT_LOSS',
      failureCode: 'CHECKOUT_ABANDONED',
      failureReason: `Cart abandoned after ${o.attempts} attempt(s). Product: ${o.notes?.product ?? 'N/A'}`,
      issuingBank: 'N/A',
      attemptsCount: 0,
      maxAttemptsAllowed: 2,
      recommendedChannel: channel,
      interventionReasoning: `Webhook: order.abandoned — ₹${Math.round(o.amount / 100).toLocaleString('en-IN')} cart drop-off`,
      totalAmountRecovered: 0,
      ...
      auditTrail: [
        {
          ...
          details: `Razorpay webhook: order.abandoned — ₹${Math.round(o.amount / 100).toLocaleString('en-IN')} cart drop-off`,
          complianceCheck: { dndCompliant: true, maxAttemptsRespected: true, disputeCheckPassed: true },
        },
      ],
    };
  ...
  // invoice.expired
  const inv = (payload as any).payload.invoice.entity;
  const channel: ActionChannel = 'WHATSAPP_UPI_LINK';
  return {
    id,
    paymentId: inv.id,
    customerName: inv.customer_details?.name ?? 'Enterprise Client',
    customerPhone: inv.customer_details?.contact ?? '+91-0000000000',
    customerEmail: inv.customer_details?.email ?? '',
    preferredLanguage: 'English',
    amountAtRisk: Math.round(inv.gross_amount / 100),
    currency: 'INR',
    leakVector: 'B2B_INVOICE',
    status: 'DETECTED',
    failureCategory: 'TECHNICAL_TIMEOUT',
    failureCode: 'INVOICE_EXPIRED',
    failureReason: `Invoice expired: ${inv.description}`,
    issuingBank: 'N/A',
    attemptsCount: 0,
    maxAttemptsAllowed: 3,
    recommendedChannel: channel,
    interventionReasoning: `Webhook: invoice.expired — ₹${Math.round(inv.gross_amount / 100).toLocaleString('en-IN')} B2B receivable`,
    totalAmountRecovered: 0,
    discountOfferedPct: 0,
    bankDowntimeDetected: false,
    createdAt: now,
    ...
    auditTrail: [
      {
        ...
        details: `Razorpay webhook: invoice.expired — ₹${Math.round(inv.gross_amount / 100).toLocaleString('en-IN')} B2B receivable`,
        complianceCheck: { dndCompliant: true, maxAttemptsRespected: true, disputeCheckPassed: true },
      },
    ],
  };
